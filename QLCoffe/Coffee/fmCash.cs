...
namespace Coffee
{
    public partial class fmCash : Form
    {
        ...
        public fmCash(int idBill, int idTable, int totalPrice, string tableName, string userName)
        {
            ...
            for (int i = 7; i <= 9; i++)
            {
                btn = new Button { Width = size, Height = size, Text = i + "", BackColor = Color.AliceBlue };
            ...
            for (int i = 1; i <= 3; i++)
            {
                if (i == 1) btn = new Button { Width = size, Height = size, Text = "0", BackColor = Color.AliceBlue }; pnPad.Controls.Add(btn);
                if (i == 2) btn = new Button { Width = size, Height = size, Text = "000", BackColor = Color.AliceBlue }; pnPad.Controls.Add(btn);
                if (i == 3) btn = new Button { Width = size, Height = size, Text = "AC", BackColor = Color.AliceBlue}; pnPad.Controls.Add(btn);
                btn.Click += (sender, args) =>
                {
                    t += (sender as Button).Text;
                    if (t.EndsWith("AC"))
        ...
        private void fmCash_Load(object sender, EventArgs e)
        {
